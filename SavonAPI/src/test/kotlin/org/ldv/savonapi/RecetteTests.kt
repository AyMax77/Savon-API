...
class RecetteTests(
    ...
    @Autowired val caracteristiquesDAO: CaracteristiquesDAO
) {
    ...
    fun setup() {
        ...
        val iode = caracteristiquesDAO.findById(1).get()
        val ins = caracteristiquesDAO.findById(2).get()
        val douceur = caracteristiquesDAO.findById(3).get()
        val lavant = caracteristiquesDAO.findById(4).get()
        val volMousse = caracteristiquesDAO.findById(5).get()
        val tenueMousse = caracteristiquesDAO.findById(6).get()
        val durete = caracteristiquesDAO.findById(7).get()
        val solubilite = caracteristiquesDAO.findById(8).get()
        val sechage = caracteristiquesDAO.findById(9).get()
        ...
        savon = Recette(
            ...
            ligneIngredients = mutableListOf(
                LigneIngredient(
                    LigneIngredientId(ingredientsId = 1, recetteId = 1),
                    quantite = 500f,
                    pourcentage = 50f,
                    ingredient = huileOlive
                ),
                LigneIngredient(
                    LigneIngredientId(ingredientsId = 2, recetteId = 1),
                    quantite = 500f,
                    pourcentage = 50f,
